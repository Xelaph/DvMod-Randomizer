...
{
	public static class JsonTranslate {
        public static string Serialize<T>(T obj) => Serialize<T>(obj, new JsonSerializerSettings());
        public static T Deserialize<T>(string json) => Deserialize<T>(json, new JsonSerializerSettings());
        public static T Deserialize<T>(string json, JsonConverter converter) => Deserialize<T>(json, FromConverters(converter));
        static JsonSerializerSettings FromConverters(params JsonConverter[] convs) {
    ...
    public class ArchipelagoSocketHelper : IArchipelagoSocketHelper
    {
        ...
        public Task ConnectAsync()
        {
			...
			_ = Task.Factory.StartNew(() =>
			{
				try
				{
					...
					_ = connectAsyncTask.TrySetResult(true);
				}
				catch (Exception e)
				{
					_ = connectAsyncTask.TrySetException(e);
				}
			});
        ...
        public void SendMultiplePackets(params ArchipelagoPacketBase[] packets)
        {
            if (webSocket != null && webSocket.IsAlive)
            {
                var packetAsJson = JsonTranslate.Serialize(packets);
